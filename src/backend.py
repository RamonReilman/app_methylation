...
def get_gene_info(annotated_bed, genes):
...
def filter_genes(gene_list, df, annotated_bed):
...
def filter_group(group_list: list[str], df: pl.DataFrame):
    return (df.filter(pl.col("group_name").is_in(group_list)))


def filter_ranges(min_range, max_range, df):
